...
test('every session can use every theme in its Standard or Fantasy group', () => {
  const themeIds = ['dark', 'high-contrast', 'light', 'high-contrast-light', 'signet', 'castle-torchlit'];
  ...
  assert.deepEqual(availableThemes().map(({ group }) => group), ['Standard', 'Standard', 'Standard', 'Standard', 'Fantasy', 'Fantasy']);
});

test('only the fantasy themes provide a wand cursor effect', () => {
  assert.deepEqual(availableThemes().map(({ cursorEffect }) => cursorEffect ?? null), [null, null, null, null, 'wand', 'wand']);
});
